...
- 导轨顶部：交通灯区域、**返回应用程序**（`返回应用`）操作，然后是**搜索设置...**药丸：
  该操作位于搜索药丸上方并返回聊天外壳。窗口过矮时其下方的目录滚动
...
- 使用当前交付状态和一项适用操作更新行：
  检查更新、查看版本或重新启动以更新
- **自动检查更新**开关行位于软件更新行正上方，持久化为可选的
  `AppSettings.autoUpdateCheck` 值（缺省即开启）。关闭后 Main 跳过启动时与
  定期的 GitHub feed 检查；手动检查（软件更新行、侧栏版本徽章、应用菜单）
  不受影响（D437）
- **开发者**卡：
  - 开发者模式关闭，除非可选持续存在
