...
| 状态 | 语义色彩 | 形状/运动 | 含义 |
|---|---|---|---|
| 待确认 | 紫色 | 受限脉冲的实心点，外加内联状态词 | 权限请求、ask 工具提问或 Plan/Goal 待批准，等待用户处理 |
| 已选择 | 中性口音 | 静态轮廓环 | 当前对话 |
| 进行中 | 橙色警告 | 呼吸脉冲受限的实心点，外加内联状态词 | 代理人正在生产或执行 |
...
两个需要关注的状态还会把状态写成文字。进行中与待确认在会话标题前渲染一个紧凑的
彩色状态词（`nav.sessionRunning` / `nav.sessionNeedsInput`），着色与圆点使用同一个
`--ds-warning` / `--ds-purple` 令牌，因此扫一眼列表读到的是文字而不是颜色解码
（D445）。该词是圆点的视觉孪生体——`aria-hidden`、文案相同、令牌相同——而安静状态
（已选择、已完成、失败）仍只有圆点，好让这个状态词始终表示「这一行在等你」。
深浅两套主题各自把文字与圆点解析到自己的令牌。
