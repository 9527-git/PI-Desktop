...
转录与侧边栏携带相同的语义颜色，每个承载状态的表面都
以视觉方式呈现其状态——而不是仅靠裸文本：

| 表面 | 状态 | 语义色彩 | 形状/运动 |
|---|---|---|---|
| 工具行（`tool-row-state`） | 运行中 | 橙色警告 | 带克制脉冲的发光标签；工具图标着警告色 |
| 工具行（`tool-row-state`） | 完成 | 成功绿色 | 带绿点的发光标签；图标保持中性 |
| 工具行 | 错误 | 错误红色 | 发光标签；圆点、图标着色与警报标签 |
| 工具行 | 拒绝 | 紫色 | 发光标签；圆点、图标着色与标签（保留给拒绝） |
| 子代理拓扑节点 | 运行中 | 橙色警告 | 带克制脉冲的头像徽标 |
| 子代理拓扑节点 | 拒绝 | 紫色 | 头像徽标（与工具行的拒绝颜色一致） |
| 子代理拓扑节点 | 中止 | 中性灰 | 头像徽标（停止是中性的，不是失败） |
| 活动组（实时） | 运行中 | 橙色警告 | 回合进行中时标题图标着色 |
| 流式回复 | 流式 | 中性口音 | 增长中的正文上闪烁的光标 |
| 工具旋转器 | 运行中 | 橙色警告 | 弧形圆环；轨道保持 30% 透明度 |

每个状态标签都以柔和的发光（低透明度的 text-shadow）携带其
语义颜色，而不是涂色的胶囊；标签仍然是唯一的完成
标记（D227）。运行中的标签保留圆点的克制脉冲并发光
警告色；拒绝保持紫色——与权限流使用的颜色相同——
因此拒绝读作权限结果而不是错误。所有运动在
`prefers-reduced-motion` 下禁用，而颜色保留。

处理行携带一种类型发光，使一次扫描就能分辨工具
家族：读取/列表/搜索/获取标签发出蓝色发光，运行标签为警告橙，
写入/编辑标签为成功绿，思考标签——加上仅思考的
活动组标签——为紫色。发光依附于转录从
`getToolAction` 派生的 `kind-*` 钩子以及活动组上的
`group-kind-*` 钩子；委托行保留其 D268 强调故事。行保持
无背景：色调存在于标签中，而从不在于行染色。聊天散文和
行摘要中的内联代码发出相同的代码蓝光，因此技术
标记无论在何处出现都以相同方式读取。聊天散文和行摘要中的粗体
发出紫色发光，因此强调读作自己的色调而不是普通墨水。
文件路径发出黄色发光——聊天散文文件链接（具有非 http href 的
`remarkChatFileLinks` 目标）和行摘要中的 `md-inline-path` 范围——
而 http(s) 链接保留中性下划线，因此路径、代码、粗体和 URL
各自一目了然地读作自己的内容类型。

行摘要文本渲染内联 markdown，而不是裸源：折叠的
工具、思考和子代理拓扑摘要通过轻量级
内联渲染器（`MarkdownInline`）运行，使代码范围和粗体以与
聊天散文相同的视觉语言读取。块级语法（列表、标题）保持
字面，单星号强调被故意不解析，因此工具参数中的 glob
模式保持原样，URL 被整体消耗，因此链接永远不会重新匹配
路径模式。

### 4. 7 Tailwind CSS 变量存根
...
15. 侧边栏会话行在标题下方显示有界的最后消息预览；其他会话的转录与
    子智能体停靠面板无需切换即可渲染（§16）
...
- **后台转录（D427）**：即使渲染器从未持有该会话，流式行也会累积到
  渲染器缓存中，因此中途打开该会话会显示其实时尾部，而不只是持久化
  读取的结果。打开时会在同一帧内提交合并后的转录。
- **子智能体停靠面板（D427）**：停靠面板属于其选择项，而不属于可见
  会话。切换会话时它保持打开，并在目标窗格保留的转录中重新找到它的
  行；只有离开聊天页面才会关闭它。
- **侧边栏预览**：每个会话行在标题下方显示最新一条用户/助手文本 ——
  由宿主截断到 120 个码点，CSS 省略号，`--text-2xs` 弱化色；转录尚
  无可读文本时不显示。
