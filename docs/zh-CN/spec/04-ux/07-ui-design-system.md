...
紧凑任务行为自己的状态标记保留一条前导**状态列**（`--ds-sidebar-status-column`，
72px），无论这一行是否带胶囊都留出同样的宽度，因此会话标题始终对齐在同一条左
边缘上（D447）。状态从来都不是
...
列表读到的是文字而不是颜色解码（D445，同组由 D446 确定，落点由 D447 确定）。胶囊
被钉在保留的状态列里——绝对定位在安静圆点一直使用的那条左边缘，居中于整行——并且
宽度被限制在该列之内，所以一枚胶囊既不会占用标题宽度，也不会因为会话开始或结束
运行而把标题推来推去。该词仍是圆点的视觉孪生体——`aria-hidden`、短到每套已发布
语言都能塞进这一列——而完整说法（`nav.sessionRunning` / `nav.sessionNeedsInput`）
仍在圆点的工具提示与可访问名称上。安静状态（已选择、已完成、失败）仍只有同一列
左边缘的圆点，没有胶囊也没有文字，好让这个状态词始终表示「这一行在等你」。
