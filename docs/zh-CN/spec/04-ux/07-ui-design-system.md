...
两个需要关注的状态还会把状态写成文字。进行中与待确认把本地化状态词
（`nav.sessionRunning` / `nav.sessionNeedsInput`）渲染在圆点自己的那枚圆角胶囊里，
胶囊底色是同一令牌的 14% 淡彩（`--ds-warning` / `--ds-purple`），因此扫一眼列表读到
的是文字而不是颜色解码（D445，落点由 D446 确定）。胶囊是行的 flex 子元素，所以即使
这一行还带着预览文字，点与词也一起居中在整行上、不会分离；该词仍是圆点的视觉孪生体
——`aria-hidden`、文案相同、令牌相同——而安静状态（已选择、已完成、失败）仍只有
留白区的圆点，没有胶囊也没有文字，好让这个状态词始终表示「这一行在等你」。
