...
- 停靠工作面板使用安静的插页纸 (`#fafafa`)，带有白色标题带和标题中的组合创建触发器，因此工具栏保留在内容上。它朝向聊天列的左缘画一条细线（D429）；面板内部仍无分隔线（D297）。
...
紧凑任务行把自己的状态标记放在行首：标记贴住行的左边缘——也就是项目头起始的
同一条左边缘——标题在它之后隔 `--ds-sidebar-status-gap`（4px）落下，因此状态与
它的标题读作一组、中间没有空档（D449）。没有标记的行，标题也从同一条左边缘
开始。状态从来都不是
...
| 状态 | 语义色彩 | 形状/运动 | 含义 |
|---|---|---|---|
| 待确认 | 紫色 | 受限脉冲的实心点，状态词与它同处一枚淡彩胶囊 | 权限请求、ask 工具提问或 Plan/Goal 待批准，等待用户处理 |
| 已选择 | 中性口音 | 静态轮廓环 | 当前对话 |
| 进行中 | 橙色警告 | 呼吸脉冲受限的实心点，状态词与它同处一枚淡彩胶囊 | 代理人正在生产或执行 |
| 已完成 | 成功绿色 | 复选标记 | 最新未读任务轮已完成 |
| 失败 | 错误红色 | 带圆圈的警报标记 | 最新未读任务转失败 |

优先级为 `待确认 → 进行中 → 已选择 → 已完成/失败`。待确认圆点是该会话上全部
人工介入来源的并集（D444）：权限队列非空、ask 工具队列非空，或存在待批准的
Plan/Goal。它的优先级高于进行中：被阻塞的智能体需要用户，而运行中的智能体只需要
时间；状态 id 与 class 仍保留 `permission` 以维持 CSS 稳定。开始另一个回合会清除
先前的最终结果；中止清除实时指示器，不造成失败。打开对话会确认其未读终端结果：
终端标记立即清除并且匹配的持久任务通知被标记为已读，因此该标记在通知刷新或重新
启动应用程序后无法返回。已标记为已读的结果永远不会产生终端标记。缩减运动模式会
禁用呼吸与脉冲动画，同时保留每个圆点的填充和本地化的可访问名称（待确认状态用
`nav.sessionNeedsInput`）。

两个需要关注的状态还会把状态写成文字。进行中与待确认把本地化状态词的短形式
（`nav.sessionRunningShort` / `nav.sessionNeedsInputShort`）渲染在圆点自己的那枚圆角
胶囊里，胶囊底色是同一令牌的 14% 淡彩（`--ds-warning` / `--ds-purple`），因此扫一眼
列表读到的是文字而不是颜色解码（D445，同组由 D446 确定，落点由 D449 确定）。胶囊
就是行按钮自己的第一个 flex 项：它的左边缘即行的左边缘——与项目头起始的那条同边——
标题在它之后隔 `--ds-sidebar-status-gap` 落下。于是状态与它的标题是一组，中间没有
死空档，整行——包括状态本身——仍是选中该会话的点击目标，也没有哪一行为一条可能
永远用不上的车道预付宽度。胶囊会先压缩自己（词截断出省略号，圆点保留自己的盒子）
再去挤压标题。该词仍是圆点的视觉孪生体——
`aria-hidden`——而完整说法（`nav.sessionRunning` / `nav.sessionNeedsInput`）
仍在圆点的工具提示与可访问名称上。安静状态（已选择、已完成、失败）仍只有同一
行首位置的圆点，没有胶囊也没有文字，好让这个状态词始终表示「这一行在等你」。
深浅两套主题各自把胶囊、圆点与文字解析到自己的令牌。
...
| 活动组（实时） | 运行中 | 橙色警告 | 回合进行中时标题图标着色、标签发光 |
| 活动组（已完成） | 完成 | 成功绿色 | 批次完成后标题标签发光 |
...
家族：读取/列表/搜索/获取标签发出蓝色发光——通用 `use` 调用与
会话 `fork` 行加入其中，因此没有工具行缺少色调——运行标签为警告橙，
写入/编辑标签为成功绿，思考标签为紫色。活动组标签陈述的是
状态而非类型：实时批次的标签在运行时发警告光，
批次完成后落定为成功绿；仅思考的组在两种状态下保持
紫色——它们的标签命名的是工作类型，而不是结果。发光依附于转录从
...
插件和通知图标按钮分组在左侧。
build/version 芯片右对齐，始终显示当前运行版本、
从不预告新版本（更新经横幅与设置 → 信息页呈现）；
单击它执行一次手动检查更新。悬停和活动状态使用语义侧边栏表面；
双方都不添加永久卡片填充。
...
页内表面不画描边（D297），仅有一处重新引入的例外：打开的工作面板朝向聊天
列的边缘（D429）。它是一条由主窗格绘制的 1px `--ds-border-default` 细线，
因为面板自身的盒宽决定原生浏览器视图的预留，画在面板上的边框会移动该预留。
页面其余内部结构仍来自三层色调加间距，边框标记只保留给浮层——那里的边缘是
层级提示而非分区。
...
英雄和可选清单
- 堆栈 (`.home-stack-inner`) 使用内容宽度 **`min(100%, 868px)`**，
  **`gap: 16px`**（工作站天花板）和自动边距以使列居中
  当视口很高时。侧边栏折叠时为 **`min(100%, 760px)`**，全屏时铺满窗格（D429）
...
| 聊天消息最大宽度 | 860px 带内的 820px 助手列 / 560px 用户板；全屏为 100%（D429） | 防止眼距过度拉伸；用户轮流保持紧凑 |
...
- 运行代理：最新助手消息左边框上的微妙脉冲 + 会话级运行/待确认状态落在侧栏会话行圆点上（D444），因此会话顶栏不再保留重复的状态指示器
...
| **最大内容宽度 820 像素** | 860px 带内的聊天消息与工具披露行（D429）——防止眼距过宽 |
