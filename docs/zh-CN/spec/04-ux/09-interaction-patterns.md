...
- 会话运行中，草稿有内容时提交槽位显示发送，草稿为空时显示停止。普通发送与回车
  发送都是后续动作：被接受后清空输入框，并把消息追加到该会话由 Host 拥有的持久
  FIFO 队列；切换会话从不移动或清空其他会话的队列。
- 队列显示在输入框上方。每一行都有两个可用键盘触达的操作：把该行放回输入框继续
  编辑的 ×，以及「立即发送」。
- 输入框已有文本或文件附件时，× 会被拒绝并弹出提示，该行保留原位。被接受时，该行
  从队列（渲染器与 Host 两侧）移除，其捕获的草稿——文本与内联文件引用——成为输入框
  内容。已被「立即发送」提升的行不可编辑。
- 「立即发送」把该行移到队首。会话运行中时，它把该行作为补充指令注入当前回合，而
  不是停止该回合：当前回复和进行中的工具继续执行，只有 Host 接受这条补充消息后该行
  才离开队列；该消息在当前回合的下一次模型请求或工具边界到达（见 §3.5）。若回合在
  接受前结束，被提升的行保持队首位置，并作为下一个回合开始。空闲时「立即发送」立即
  发出。
- 不使用「立即发送」时，下一行 FIFO 在当前回合完成、失败或被中止后自动开始。终止
  事件可能先于持久化释放会话到达；最终化必须在释放归属后再次唤醒队列。无需额外发送
  或切换会话。
- 中止始终立即生效，且从不清空队列。排队的提示在应用重启后仍然存在，并在控制器
  接入前保持等待（ADR 0213）。应用关闭期间进行最终化不得启动另一个排队回合。
...
- 上下文使用检查器为最新的检查点保留一条静音线，
  在面板打开时显示；计数、摘要成本，以及最新检查点记录携带该值时
  被替换掉的上下文占用（“本次压缩前占用”）位于紧凑的模型/工具摘要下方，
  不再添加解释性文案。从卡片内启动压缩后卡片保持打开：忙碌文案、
  更新后的检查点行与压缩前占用即为原地反馈。
