...
- **前提条件**：一个已配置模型且工作区可写的会话。转录将展示混合的处理行（若干读取、一次搜索、一次运行、一次写入、一行思考）、一个被拒绝的工具调用、一个失败的命令，以及一个正文中带有粗体、内联代码、一个工作区文件链接和一个 http(s) 链接的回复。至少一个工具参数包含 glob（例如 `src/**/*.ts`）。
- **步骤**：运行一个产生上述行的回合，然后在两种主题下检查转录。读取工具行旁的状态标签、仅思考活动组的标签，以及工具行、思考行和子代理拓扑节点的折叠摘要。检查回复正文中的粗体、代码、路径与 URL 处理。在开启缩减运动的情况下重新运行失败的命令。
- **预期**：每个状态标签都以其语义颜色渲染为发光文本——运行中的行为警告色、完成的行为成功色、失败的为错误色、被拒绝的为紫色——标签后方没有填充的胶囊；运行中的圆点保留其克制的脉冲，缩减运动模式保留颜色并移除动画。处理行只按类型为其行名着色：读取/列表/搜索/获取发光蓝色，运行发光警告橙，写入/编辑发光成功绿，思考行发光紫色；仅思考的活动组标签发光紫色，而混合组保持中性。行名没有背景染色，类型颜色在浅色与深色主题下都保持可读。折叠摘要渲染内联 markdown：代码范围发光蓝色、粗体发光紫色、文件路径发光黄色，而 http(s) 链接保留中性下划线；块级语法与工具参数中的 glob 模式保持字面。回复正文携带相同的发光语言。
