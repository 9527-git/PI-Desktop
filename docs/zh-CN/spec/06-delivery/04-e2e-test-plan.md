...
- **步骤**：运行一个产生上述行的回合，然后在两种主题下检查转录。读取工具行旁的状态标签、仅思考活动组的标签，以及工具行、思考行和子代理拓扑节点的折叠摘要。检查回复正文中的粗体、代码、路径与 URL 处理。在长时间运行的命令、子代理扇出和流式回复仍处于活动状态时，检查其图标、圆点、徽标与光标状态。在开启缩减运动的情况下重新运行失败的命令。
- **预期**：每个状态标签都以其语义颜色渲染为发光文本——运行中的行为警告色、完成的行为成功色、失败的为错误色、被拒绝的为紫色——标签后方没有填充的胶囊。状态从不只靠文本表达：运行中的行显示其脉冲警告圆点、着警告色的工具图标和警告色旋转弧；失败的行显示红色圆点、警报标签与红色图标着色；被拒绝的行显示紫色圆点、紫色图标着色与拒绝标签；完成的行只显示其绿点且图标保持中性。实时的子代理拓扑节点为其头像加上同样的警告脉冲徽标，被拒绝的节点携带紫色徽标，被中止的节点保持中性灰；实时活动组即使内部所有行都已折叠也为标题图标着警告色。流式回复在增长中的正文上保留闪烁的光标；缩减运动模式保留所有颜色并移除所有动画。处理行只按类型为其行名着色：读取/列表/搜索/获取发光蓝色，运行发光警告橙，写入/编辑发光成功绿，思考行发光紫色；仅思考的活动组标签发光紫色，而混合组保持中性。行名没有背景染色，类型颜色在浅色与深色主题下都保持可读。折叠摘要渲染内联 markdown：代码范围发光蓝色、粗体发光紫色、文件路径发光黄色，而 http(s) 链接保留中性下划线；块级语法与工具参数中的 glob 模式保持字面。回复正文携带相同的发光语言。
