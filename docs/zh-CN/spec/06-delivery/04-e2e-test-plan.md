...
- 会话列表刷新或模型能力查询：`pnpm test:e2e` 和 `pnpm test:e2e:boot`，
  包括合成大列表的响应性检查。
- 单个提供商模型选择：`pnpm test:e2e:provider-model-selection`
  （真实 React/Chromium 指针与键盘激活，提供商 API 打桩）。
...
#### E2E-011f：运行中发送按会话排队并支持立即发送

- **先决条件**：已配置提供商；会话 A 能产生带有至少一个已完成工具批次的
  延迟响应；会话 B 存在且空闲。
- **步骤**： 1) 在 A 发送一条长时间运行的提示。2) 当 A 运行时，确认
  输入框只有一个提交按钮：草稿为空时是 Stop
  （`aria-label="Stop generating"`），且不存在 Send 按钮；输入一段草稿
  并确认同一位置变为 Send（`aria-label="Send"`），且不存在 Stop
  按钮。再发送两条提示并检查输入框上方的队列。3) 在输入框为空时，点击
  第二条排队行的 ×，确认其文本与文件引用回到输入框、该行离开队列；
  编辑草稿，然后输入新的文本并点击另一条排队行的 ×，确认编辑被 toast
  拒绝且该行留在队列中；再次清空输入框并切换到 B。4) 在 B 发送一条提示，
  然后在任一运行结束前返回 A。5) 对 A 剩下的排队行选择立即发送。
  6) 观察 A：该提示在下一个工具/回复边界作为转向注入运行中的回合而不
  停止回复，且只有 Host 接受注入后该行才离开队列。7) 在 A 开始另一次
  运行，清空草稿以露出单个 Stop 按钮，按 Stop，并检查队列。8) 用两条
  排队提示重复，让活动回合在没有立即发送的情况下完成，并延迟其宿主
  `session.endTurn` 响应直到 `agent_end` 投递之后。释放终结并观察两条
  后续提示穿过各自的回合边界。用提供商错误与立即中止重复。
- **预期**：单一提交位置在每种状态下都恰好包含一个按钮：空闲且为空时
  禁用的 Send，运行中且有内容时启用的 Send（把提示排队），以及运行中且
  草稿为空时的 Stop。A 的两条提示按 FIFO 顺序出现，B 的队列保持独立。
  点击排队行的 × 会把其文本与文件引用退回空输入框以便重新编辑，该行
  离开队列；当输入框已有草稿时编辑被 toast 拒绝，该行留在队列中。立即
  发送在下一个工具/回复边界把选中的提示作为转向注入运行中的回合，不停止
  回复或当前工具；只有 Host 接受注入后该行才离开队列，而在接受前结束的
  回合会让被提前的行改为作为下一回合启动。立即 Stop 中止当前回复并保留
  A 的排队行；切换会话保留两个队列。普通完成、提供商失败与中止都会在
  持久化终结释放会话后自动恢复排队发送。终结挂起时没有排队提示会启动；
  后续每个回合按 FIFO 顺序只启动一次，无需再次点击或切换会话。重复的
  终止处理不会重复派发。其他会话的队列保持不变，终结挂起时退出会保留
  排队工作而不启动另一个回合。
- **链接规格**：`03-runtime/01-ipc-protocol.md` (§5.2)，
  `04-ux/08-component-spec.md` (§11)，
  `04-ux/09-interaction-patterns.md` (§3.4)，ADR 0118，ADR 0213，ADR 0259
- **接受**：C（聊天、流与会话隔离），质量
- **里程碑**：M6+
- **状态**：源级回归与确定性的桌面终结 / Agent Host 集成已覆盖
  （`queued-turn-finalization.test.mjs`）；完整 UI 场景草稿
...
- **步骤**：1) 每种状态各种植一行——选中、运行中、权限、ask、Plan/Goal 审批——
  每行都带预览文字，使行高与真实会话一致（两行）。
  2) 检查每行的前导状态圆点、它所在的胶囊、胶囊内的状态词，并把该标记在行上的位置
  与一级项目标头的左边缘、与该行标题自身的左边缘放在一起量。
  3) 在明暗两种主题下重复运行中与待确认两行。
...
- **预期**：运行中行显示橙色呼吸圆点（`--ds-warning`，
  ...
  在此之上，按 D445（分组由 D446 确定）：运行中行与三个待确认行把状态词放在
  圆点自己那枚淡彩胶囊里——胶囊同时装着圆点与文字，圆点在其中为静态定位，
  胶囊确有底色，且即使该行有两行高，这一组的中心仍与整行的垂直中心相差不超过
  2px，因此文字不可能漂到标题那一行上。文字是该圆点可访问名称的短形式（不长于
  它、也不被截断），并使用圆点自身的令牌（运行中为 `--ds-warning`，全部介入来源
  为 `--ds-purple`，
  两种主题皆如此）；而选中行没有胶囊、没有文字，同一个前导位置仍是原本那个普通
  圆点，标题照常渲染。
  按 D449，标记改为领先所在行而不是站在一条自己的车道里：每枚标记的左边缘都顶在
  行的左边缘上，不会比一级项目标头自身的左边缘更靠左，且它的标题在标记右边缘起
  6px 内开始——因此状态与标题读起来是一个组、中间没有死区，也没有任何一行为
  一个可能永远用不到的标记预留宽度。每枚标记同时属于行自己的按钮，所以点它就能
  选中该会话。
...
- **状态**：自动化（`scripts/e2e-sidebar-status-dot.mjs`，经
  ...
  明暗主题令牌、预留状态列与共享标题边缘、减少动效，以及顶部栏缺失探针）；单元覆盖见
...
- **步骤**： 1) 将脚本滚动到最新消息。 2) 测量
  ...
  输入框（不重叠）。 5) 将草稿折叠回一行并
  确认差距缩小至约 16 像素储备。
- **预期**：最后一条消息位于输入框上方（一个小的、一致的间隙），而不是远远低于它；正常流高度使更高的多行草稿缩小转录本视口而不是覆盖它。跳转到最新按钮和小地图在每个草稿高度都固定在输入框上方。
...
| 品质 | E2E-032、E2E-033、E2E-039、E2E-043、E2E-044、E2E-045、E2E-046、E2E-047、E2E-048、E2E-048A、E2E-049、E2E-050、E2E-053、E2E-055、 E2E-056、E2E-057、E2E-058、E2E-059、E2E-060、E2E-061、E2E-062、E2E-063、E2E-064、E2E-065、E2E-066、E2E-067、E2E-068、E2E-069、 E2E-070、E2E-071、E2E-072、E2E-073、E2E-074、E2E-075、E2E-076、E2E-077、E2E-078、E2E-079、E2E-080、E2E-081、E2E-082、E2E-083、 E2E-084、E2E-085、E2E-086、E2E-092、E2E-093、E2E-094、E2E-095、E2E-096、E2E-097、E2E-098、E2E-099、E2E-100、E2E-101、E2E-102、 E2E-102a、E2E-102b、E2E-103、E2E-AGENTS-001、E2E-024N、E2E-024O、E2E-059a、E2E-060b、E2E-060c、E2E-060d、E2E-061a、E2E-073a、E2E-111、 E2E-114、E2E-117、E2E-118、E2E-119、E2E-120、E2E-122、E2E-123、E2E-142、E2E-143、E2E-144、E2E-145、E2E-146、E2E-147、E2E-148、E2E-150、E2E-151、E2E-153、E2E-194、E2E-195、E2E-199、E2E-200、E2E-201、E2E-202、E2E-203、E2E-204、E2E-209、E2E-210、E2E-250、E2E-PLUGIN-imported-pi-package-skills、E2E-CHAT-fullscreen-widens-reading-band、E2E-WORKPANEL-open-dock-draws-chat-column-seam |
| 品质（项目排序） | E2E-253 |
...
| 品质（跨会话渲染） | E2E-CHAT-other-sessions-render-without-switching |
...
- 在 1200×690 光空的家里，输入框板外部宽度为〜868px（不是〜640px）。
- 主页建议网格与输入框板跨越相同的内容列。
...
- 打开设置灯光主题~1200×690。
- 全页外壳：`#f3f3f3` 上的轨道 ~260px，主 `#fff`；返回应用程序位于导轨顶部、搜索
  药丸上方；带有图标的常规活动药丸。
...
- **步骤**：1) 打开设置 → 模型配置并重新打开该提供商。2) 在实时模型列表中拖选一个模型 id 并复制；确认剪贴板是该 id，且行复选框没有变化。3) 不选中文本时点击同一行文本、行内空白和复选框，并在复选框聚焦时按空格；确认绑定保持不变且其配置行打开。4) 展开某行高级选项并输入短别名。5) 保存后打开 Composer 模型选择器；确认该模型显示别名，另一行仍用目录名。6) 分别用别名和真实 id 搜索，都能找到该行。7) 清空别名并保存，确认恢复目录显示名。8) 再填别名、保存、重开提供商并重启应用，别名仍在。9) 在别名输入框输入超过 60 个字符，确认只保留前 60 个；再直接用 `providers.update` 发送 61 字符别名，确认失败码为 `MODEL_ALIAS_TOO_LONG`。10) 依次点击该已配置行的 id、行内空白与上限数值单元格，确认每次点击后模型仍保持已配置、其高级区已展开，且右栏滚动到了该行。11) 点击一行尚未配置模型的名称，确认该行复选框被勾选、其配置行出现。
- **预期**：别名只是展示标签，请求仍携带 `models[].id`；配置行在别名芯片旁继续显示真实 id。模型 id 和名称在禁止选择的外壳内仍可选择，行内拖选只是复制。激活单个模型（含复选框）永不删除其配置——已配置的模型会打开其配置行并短暂标记，尚未配置的会被选中，唯一删除途径是右侧面板的显式「移除」操作。空白或清空的别名回退到目录显示名。
- **链接规格**：`03-runtime/12-provider-config-schema.md`、`04-ux/08-component-spec.md`、ADR 0192
- **验收**：B（模型配置）、质量
- **里程碑**：M2
- **状态**：单元已覆盖（`composer-models.test.mjs`、`provider-model-config.test.mjs`）；提供商编辑器的复制/选择/保存载荷由 `pnpm test:e2e:provider-model-selection` 以打桩 API 覆盖。Composer 显示、真实剪贴板、host 校验与重启仍为手动。

#### E2E-PROVIDER-select-all-toggles-visible-models：头部全选切换全部可见行

（自 E2E-PROVIDER-select-all-never-clears 更名——它本身又自
E2E-PROVIDER-fallback-hides-select-all（D438）更名——D443 在 D442 已持久化取消
勾选绑定之后，把复选框改成了真正的切换。）

- **前提条件**：一个提供商的实时模型探测失败或返回空列表，编辑器左列表渲染 fallback 提示（`modelsFallbackNote`），可见行恰好就是已配置的模型；至少保存了两个模型绑定，其中一个带别名、上限或思考等级。
- **步骤**：1) 打开设置 → 模型配置并编辑该提供商。2) 确认左列表显示 fallback 提示，列表标题旁渲染的全选复选框处于勾选态（每个已配置行都已选中）。3) 取消勾选全选：每个可见绑定从右侧面板移除，但每一行仍保留为未勾选状态，头部变为未勾选。4) 重新勾选全选：每一行恢复为已选，其保存的别名、上限与思考等级完好。5) 输入搜索过滤隐藏一个已配置模型，再取消勾选全选：只有可见行被清空，被隐藏的已配置模型保留绑定。6) 保存并重开：留为未勾选的行仍以未勾选状态列出（D442），重新勾选恢复其参数。7) 通过右侧面板的显式「移除」删除一个模型，保存并重开：被删模型的行保持隐藏（D441），直到通过隐藏清单的「显示」条目或自定义重加恢复。
- **预期**：头部全选是真正的切换（D443）：勾选添加全部可见行，取消勾选软移除它们——每个被移除的绑定都被记住（持久化在 `disabledModels`）、其行保留为未勾选，因此 fallback 批量清空再也不可能丢掉已配置模型。这取代了 D440 的只加不删规则，该规则的存在只是因为当时取消勾选的绑定尚未持久化。右侧面板的显式「移除」仍是唯一的破坏性路径，它经 `hiddenModels` 隐藏被删的行（D441）。
- **链接规格**：`04-ux/08-component-spec.md` §19.4、ADR 0192、D440、D441、D442、D443
- **验收**：B（模型配置）
- **里程碑**：M2
- **状态**：源合约已覆盖（`provider-model-selection-safe.test.mjs`）；UI 旅程已通过 CDP 对用户数据副本实测（D438 复现），切换行为复用同一旅程。

#### E2E-PROVIDER-copy-endpoint-and-key：提供商编辑器快捷复制

- **前提条件**：一个保存了 API 密钥的具名 AI 服务（以及一个负例：没有已存密钥的提供商）。
- **步骤**：1) 编辑该提供商，确认接口地址行带复制按钮。2) 点击并在别处粘贴：剪贴板是完整 base URL。3) 确认编辑态的 API 密钥输入框带复制按钮。4) 点击并粘贴：剪贴板是已存密钥，且出现成功 toast。5) 对没有已存密钥的提供商点击密钥复制按钮：出现缺失提示 toast，剪贴板保留原有内容。
- **预期**：接口地址复制经渲染进程剪贴板写入完整 base URL。密钥复制绝不把秘密送到渲染进程：`providers.copySecret` 主进程通道自行从 host-core 读取已存密钥并写入 Electron 剪贴板，值不可能经渲染进程状态或 devtools 泄漏；缺少密钥时提示缺失。
- **链接规格**：`04-ux/08-component-spec.md` §19.4、D440
- **验收**：B（模型配置）
- **里程碑**：M2
- **状态**：手动；主进程边界（值不经 IPC 返回）由 `packages/shared/src/protocol.ts` 的渠道契约固定。

#### E2E-PROVIDER-row-toggle-and-hidden-delete：行复选框真切换，「移除」真删除

- **前提条件**：在设置 → 模型配置中编辑一个提供商，其实时探测列出至少三个模型；其中至少一个配置了别名、上下文窗口上限或思考等级。另有第二个提供商（或同一提供商把端点改坏后）实时探测失败或返回为空，左列表由已配置绑定单独构成（fallback），其中包含一个手动添加的模型 id。
- **步骤**：1) 取消勾选某个已配置模型的行复选框：右侧面板的绑定被移除，但该行仍以未勾选状态保留在左列表。2) 关弹窗前重新勾选同一行：原绑定连同别名、上限与思考等级一并恢复。2a) 在 fallback 提供商上取消勾选一个已配置（或手动添加）的模型：该行以未勾选状态保留，头部全选会按保存参数把它重新加回；行绝不消失。2b) 保存并重开该提供商：未勾选的行仍以未勾选状态保留；全选或点行会按保存参数恢复它。3) 不保存直接关闭，重开：确认持久化状态未被改动。4) 用右侧「移除」删除另一个已配置模型：绑定被移除，其发现行从左列表消失，列表下方出现「已隐藏 N 个 · 显示」条目。5) 保存并重开：即使服务仍在播报该模型，隐藏行也不出现。6) 点「显示」：该行以未勾选状态恢复，重新勾选即可再配置。7) 重复步骤 4，然后在自定义模型输入框手输该隐藏 id：行被解除隐藏并完成配置。
- **预期**：行复选框是真正的切换（D441）：取消勾选只移除绑定、行保留在列表中，并在本次编辑会话内记住被移除的绑定，重新勾选即可恢复其参数——在每种发现模式下都成立，包括 fallback 列表与手动添加的模型：记住的绑定会合并回显示行。自 D442 起这份记忆随提供商记录持久化在 `disabledModels` 集合中，未勾选的行在保存重开后仍在；删除会丢弃这份记忆。右侧「移除」是真正的删除：它移除绑定并通过 `providers.create` / `providers.update` 把 id 持久化到提供商的 `hiddenModels` 集合、隐藏发现行，因此保存重开后模型不会重新出现。「显示」条目一次性恢复全部隐藏行，手输隐藏 id 也会解除隐藏；隐藏 id 永远不会隐藏仍持有绑定的行。
- **链接规格**：`04-ux/08-component-spec.md` §19.4、D441
- **验收**：B（模型配置）
- **里程碑**：M2
- **状态**：源合约已覆盖（`provider-model-selection-safe.test.mjs`、`provider-model-config.test.mjs`）；提供商 config JSON 的持久化由 host-core 的 create/update 路径承载。UI 旅程与重开行为在运行前保持手动。
...
### E2E-QUEUE-prompt-during-manual-compaction-is-queued-and-delivered：手动压缩期间发送的提示会排队并在压缩后投递

- **前提条件**：一个使用缓慢摘要提供程序的会话，使手动压缩在进行中停留足够久，足以输入并发送提示；会话空闲，没有活动回合，也没有排队中的提示。
- **步骤**：在输入框运行 `/compact`，并在压缩指示可见期间发送一条提示。在压缩给出结果前检查 FIFO 行与运行时状态；然后让压缩完成并让排队的提示运行。再用以下情形重复：压缩因提供程序错误而失败、提示排队期间按下 Stop、以及同一次压缩期间发送两条提示。
- **预期**：手动压缩期间发送的提示会作为普通 FIFO 行排队而不是立即开始：不出现 `AGENT_BUSY` 错误行，草稿也不会被还原到输入框。压缩给出结果后，排队的提示自行开始并在一回合内完成。压缩失败仍会释放占用并排空队列；压缩结束前被移除的排队提示永不开始；两条提示保持 FIFO 顺序。转录中压缩行出现在排队回合之前。
- **链接规格**：`03-runtime/02-agent-runtime.md`（§5）、`03-runtime/01-ipc-protocol.md`（prompt/compact 准入）
- **状态**：已记录；集成到 main 后运行。

### E2E-CHAT-external-path-open-reveal：聊天中的工作区外路径可打开、可显示并报告失效

- **前提条件**：一个已配置模型的工作区，以及工作区外一个已存在的本地文件（如构建产物 `E:\out\report.xlsx`）和一个不存在的路径。应用数据目录中是实时数据库。
- **步骤**：让智能体报告一个工作区外的产物路径，然后点击渲染出的路径及其「在文件夹中显示」操作。再用以下情形重复：目录、含空格的路径、带 `:line` 后缀的路径、行渲染之后文件被删除的失效路径，以及应用数据目录下的路径。检查 `fs/open`、`fs/reveal` 和 `fs/stat` 调用，覆盖工作区相对路径、工作区内绝对路径、会话 scratch blob 与工作区外绝对路径。
- **预期**：盘符路径、反斜杠分隔符与 MSYS 挂载点被识别为文件引用；工作区外的绝对路径解析为外部目标并经操作系统打开，而不是交给工作面板查看器。`fs/open` 与 `fs/reveal` 在 realpath 加保护根目录门禁之后接受该已存在的工作区外绝对路径，同时工作区与会话根包含检查保持优先且行为不变；相对路径、`~` 路径和缺失目标仍被拒绝，应用数据目录下的路径即使经由植入的符号链接也会被拒绝。显示操作会在文件夹中选中文件、打开目录，并在条目消失时打开最近的已存在祖先目录，因此绝不会成为静默无操作。`fs/stat` 的结论只报告 `{ exists, kind }` —— 不含内容、大小或列表 —— 失效路径报告 `exists: false`，据此标记该行并禁用其操作。`fs/read`、`fs/list` 与 `fs/index` 仍拒绝工作区与会话根之外的任何路径。
- **链接规格**：`03-runtime/01-ipc-protocol.md`；ADR 0256；ADR 0257；D320 / D322。
- **状态**：已记录；集成到 main 后运行。

### E2E-CHAT-turn-summary-card：每个已完成回合以摘要卡片收尾

- **前提条件**：一个已配置模型且工作区可写的会话。一个回合编辑一个受跟踪的文件、新增一个文件并删除一个；另一个回合只读取；第三个回合在最终回答中报告一个没有工具写入的产物路径（构建产物），其中一个文件在检查该行之前被删除。
- **步骤**：运行每个回合，检查已完成回合下方出现的卡片。展开并折叠某个已变更文件的 diff，使用该行的工作面板打开操作与文件管理器显示操作，检查文件已被删除的路径，然后切换会话并重新打开转录。重载渲染器并再次检查卡片。再用以下情形重复：没有改变任何内容的回合，以及最终回答报告路径超过行数上限的回合。
- **预期**：每个已完成回合只渲染一张摘要卡片，位于该回合自身的行之下，且其上没有属于下一个回合的内容；回合仍处于活动状态时不渲染卡片，输入框中也不渲染。变更表把每个被持久变更的文件列一次，按新增/修改/删除分组，操作与 +/− 计数正确，其 diff 可展开可折叠。产出表列出最终回答报告的路径，包括没有工具写入的产物。路径不再可解析的行显示本地化的未找到提示并禁用其操作；可解析路径在工作面板打开并在文件管理器中显示；工作区外的路径经操作系统打开。没有改变任何内容的回合显示空变更标签而不是表格。卡片在会话切换与渲染器重载后不重复也不丢失行，所有已发布语言环境都能渲染这些标签。
- **链接规格**：`04-ux/08-component-spec.md`；ADR 0256；ADR 0257；D425。
- **状态**：已记录；集成到 main 后运行。

### E2E-CHAT-transcript-glow-language：转录状态以语义发光标签呈现

- **前提条件**：一个已配置模型且工作区可写的会话。转录将展示混合的处理行（若干读取、一次搜索、一次运行、一次写入、一行思考）、一个被拒绝的工具调用、一个失败的命令，以及一个正文中带有粗体、内联代码、一个工作区文件链接和一个 http(s) 链接的回复。至少一个工具参数包含 glob（例如 `src/**/*.ts`）。一条长时间运行的命令、一个实时子代理扇出和一个流式回复在其行被检查期间保持进行中。
- **步骤**：运行一个产生上述行的回合，然后在两种主题下检查转录。读取工具行旁的状态标签、实时混合组、已完成混合组与仅思考组的标签——折叠实时组以读取其活动走马灯——以及工具行、思考行和子代理拓扑节点的折叠摘要。检查回复正文中的粗体、代码、路径与 URL 处理。在长时间运行的命令、子代理扇出和流式回复仍处于活动状态时，检查其图标、圆点、徽标与光标状态。在开启缩减运动的情况下重新运行失败的命令。
- **预期**：每个状态标签都以其语义颜色渲染为发光文本——运行中的行为警告色、完成的行为成功色、失败的为错误色、被拒绝的为紫色——标签后方没有填充的胶囊。状态从不只靠文本表达：运行中的行显示其脉冲警告圆点、着警告色的工具图标和警告色旋转弧；失败的行显示红色圆点、警报标签与红色图标着色；被拒绝的行显示紫色圆点、紫色图标着色与拒绝标签；完成的行只显示其绿点且图标保持中性。实时的子代理拓扑节点为其头像加上同样的警告脉冲徽标，被拒绝的节点携带紫色徽标，被中止的节点保持中性灰；实时活动组即使内部所有行都已折叠也为标题图标着警告色。流式回复在增长中的正文上保留闪烁的光标；缩减运动模式保留所有颜色并移除所有动画。处理行只按类型为其行名着色：读取/列表/搜索/获取发光蓝色——通用 `use` 调用与 `fork` 行加入其中——运行发光警告橙，写入/编辑发光成功绿，思考行发光紫色。活动组标签陈述的是状态而非类型：实时混合组或子代理组的标签在运行时发警告光，批次完成后落定为成功绿；仅思考的组标签在两种状态下保持紫色。行名没有背景染色，类型颜色在浅色与深色主题下都保持可读。折叠摘要——包括运行中组标题下方的实时活动走马灯——渲染内联 markdown：代码范围发光蓝色、粗体发光紫色、文件路径发光黄色，而 http(s) 链接保留中性下划线；块级语法与工具参数中的 glob 模式保持字面。回复正文携带相同的发光语言。
- **链接规格**：`04-ux/07-ui-design-system.md` §4.6；D227；D268。
- **状态**：已记录；集成到 main 后运行。

### E2E-CHAT-other-sessions-render-without-switching：其他会话的内容无需切换即可渲染

- **前提条件**：两个或更多会话；一个会话在另一个会话被选中时运行回合。正在运行的会话中打开了子智能体详情。
- **步骤**：1) 在选中会话 A 时，从第二个入口在会话 B 中启动一个长回合。2) 检查侧边栏：会话 B 的行在标题下方显示其最新用户/助手文本的一行预览。3) 在运行中切换到会话 B，确认实时尾部渲染且没有重建空屏。4) 在会话 B 中打开子智能体详情，然后切回会话 A，确认停靠面板仍显示所选的子智能体。
- **预期**：即使渲染器从未持有该会话，后台会话的流式行也会累积到渲染器缓存中，因此中途打开它会显示实时行，而不只是持久化读取的结果。子智能体停靠面板属于其选择项，而非可见会话：切换会话时停靠面板保持打开，并在目标窗格保留的转录中重新找到它的行；只有离开聊天页面才会关闭它。侧边栏行显示有界的最后消息预览（由宿主截断到 120 个码点、CSS 省略号、弱化的 `--text-2xs`），转录尚无可读文本时不显示。
- **链接规格**：`04-ux/07-ui-design-system.md` §16；D427。
- **状态**：已记录；集成到 main 后运行。

### E2E-CHAT-fullscreen-widens-reading-band：全屏让聊天带铺满窗格

- **前提条件**：PI-Desktop 已打开，并有一个活动聊天会话，其转录与输入框渲染在居中带上；工作面板可开可关。缩减运动已关闭。
- **步骤**：1) 记录转录带与输入框堆栈的渲染宽度。2) 进入窗口全屏（F11、视图菜单或平台全屏控件）。3) 检查转录带、消息列与输入框堆栈。4) 在侧栏折叠状态下重复一次。5) 退出全屏并检查恢复后的带。
- **预期**：全屏时聊天带取消上限并铺满主窗格：转录、其消息行与输入框共享同一个全窗格包络，转录内缩与输入框装订边仍在窗格边缘可见。折叠侧栏的上限不会延续到全屏。退出全屏后恢复与当前侧栏状态相符的展开或折叠上限，两种过渡期间都没有内容溢出或裁切。
- **链接规格**：`04-ux/08-component-spec.md`、`04-ux/07-ui-design-system.md`、`04-ux/01-ui-ia.md`
- **验收**：质量
- **里程碑**：M5
- **状态**：单元覆盖（`sidebar-collapse-animation.test.mjs`）；渲染交互场景草稿

### E2E-WORKPANEL-open-dock-draws-chat-column-seam：打开的停靠面板朝向聊天列画一条细线

- **前提条件**：PI-Desktop 已打开一个聊天会话，客户端宽度允许工作面板与主窗格并存。
- **步骤**：1) 在工作面板关闭时检查聊天列的右缘。2) 打开工作面板。3) 检查聊天列与停靠面板之间的边界，包括标题带。4) 将停靠面板切换到浏览器工具（原生插件视图）并检查同一边界。5) 进入预览模式（最大化停靠面板）并检查窗口左缘；然后退出预览模式。6) 关闭面板。
- **预期**：恰好一条 1px 细线将聊天列与停靠面板分开，从 shell 顶部画到底部，位于顶栏带之上且不被原生插件视图中断。停靠面板开合动画期间它贴着边界移动。没有停靠面板挂载时不画接缝，预览模式在窗口边缘不显示接缝。缩放把手的悬停线仍在接缝之上可读，接缝从不拦截指针输入。
- **链接规格**：`04-ux/08-component-spec.md`、`04-ux/07-ui-design-system.md`
- **验收**：质量
- **里程碑**：M5
