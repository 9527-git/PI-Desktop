...
#### E2E-PROVIDER-select-all-never-clears：头部全选只加不删

（自 E2E-PROVIDER-fallback-hides-select-all（D438）更名：D440 推翻了隐藏方案，
复选框恢复常显但改为只加不删。）

- **前提条件**：一个提供商的实时模型探测失败或返回空列表，编辑器左列表渲染 fallback 提示（`modelsFallbackNote`），可见行恰好就是已配置的模型；至少保存了两个模型绑定。
- **步骤**：1) 打开设置 → 模型配置并编辑该提供商。2) 确认左列表显示 fallback 提示，且列表标题旁渲染了全选复选框。3) 取消勾选全选框，确认没有任何模型被移除：每个已配置模型保持勾选（取消勾选是空操作）。4) 输入搜索过滤隐藏一个已配置模型，再勾选全选：只有可见行受影响，被隐藏的已配置模型保留绑定。5) 逐一点击左列表的每一行（id、行内空白、复选框），确认每个已配置模型保持勾选、没有任何模型被移除。6) 通过右侧面板的显式「移除」删除一个模型，保存并重开：其余绑定仍在，且被删模型的行保持隐藏（D441），直到通过隐藏清单的「显示」条目或自定义重加恢复。
- **预期**：头部全选在所有模式下都只加不删（D440）：它能添加可见行，但没有清空路径，因此 fallback 列表（恰好就是已配置的模型）绝不可能被引发回归的那个控件批量清空。删除只保留右侧面板的显式移除，它会隐藏被删的行（D441），因此 fallback 绑定不会在没有任何恢复路径的情况下丢失。
- **链接规格**：`04-ux/08-component-spec.md` §19.4、ADR 0192、D440、D441
- **验收**：B（模型配置）
- **里程碑**：M2
- **状态**：源合约已覆盖（`provider-model-selection-safe.test.mjs`）；UI 旅程已通过 CDP 对用户数据副本实测（D438 复现），只加不删改版复用同一旅程。
