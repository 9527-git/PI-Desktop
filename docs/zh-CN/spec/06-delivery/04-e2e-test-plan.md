...
#### E2E-PROVIDER-fallback-hides-select-all：fallback 列表不提供批量清空

- **前提条件**：一个提供商的实时模型探测失败或返回空列表，编辑器左列表渲染 fallback 提示（`modelsFallbackNote`），可见行恰好就是已配置的模型；至少保存了两个模型绑定。
- **步骤**：1) 打开设置 → 模型配置并编辑该提供商。2) 确认左列表显示 fallback 提示，且列表标题旁没有渲染全选复选框。3) 逐一点击左列表的每一行（id、行内空白、复选框），确认每个已配置模型保持勾选、没有任何模型被移除。4) 通过右侧面板的显式「移除」删除一个模型，保存并重开：其余绑定仍在。
- **预期**：fallback 模式下可见行就是已配置的模型本身，头部全选框在全员已选的稳态下唯一可能的动作就是把实时列表可能再也不会给出的绑定批量清空，因此不渲染该控件。删除只保留右侧面板的显式移除；如此删除的模型在 fallback 模式下可能不再出现在列表里，这正是 fallback 模式不提供批量破坏性控件的原因。
- **链接规格**：`04-ux/08-component-spec.md` §19.4、ADR 0192、D438
- **验收**：B（模型配置）
- **里程碑**：M2
- **状态**：源合约已覆盖（`provider-model-selection-safe.test.mjs`）；UI 旅程已通过 CDP 对用户数据副本实测。
