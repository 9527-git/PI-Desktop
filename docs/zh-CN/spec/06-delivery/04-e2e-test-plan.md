...
（自 E2E-PROVIDER-fallback-hides-select-all（D438）更名：D439 推翻了隐藏方案，
复选框恢复常显但改为只加不删。）
...
- **预期**：头部全选在所有模式下都只加不删（D439）：它能添加可见行，但没有清空路径，因此 fallback 列表（恰好就是已配置的模型）绝不可能被引发回归的那个控件批量清空。删除只保留右侧面板的显式移除；如此删除的模型在 fallback 模式下可能不再出现在列表里，这正是任何位置都不提供批量破坏性控件的原因。
- **链接规格**：`04-ux/08-component-spec.md` §19.4、ADR 0192、D439
- **验收**：B（模型配置）
- **里程碑**：M2
...
- **链接规格**：`04-ux/08-component-spec.md` §19.4、D439
- **验收**：B（模型配置）
- **里程碑**：M2
