...
- **步骤**：1) 取消勾选某个已配置模型的行复选框：右侧面板的绑定被移除，但该行仍以未勾选状态保留在左列表。2) 关弹窗前重新勾选同一行：原绑定连同别名、上限与思考等级一并恢复。2a) 在 fallback 提供商上取消勾选一个已配置（或手动添加）的模型：该行以未勾选状态保留，头部全选会按保存参数把它重新加回；行绝不消失。3) 不保存直接关闭，重开：确认持久化状态未被改动。4) 用右侧「移除」删除另一个已配置模型：绑定被移除，其发现行从左列表消失，列表下方出现「已隐藏 N 个 · 显示」条目。5) 保存并重开：即使服务仍在播报该模型，隐藏行也不出现。6) 点「显示」：该行以未勾选状态恢复，重新勾选即可再配置。7) 重复步骤 4，然后在自定义模型输入框手输该隐藏 id：行被解除隐藏并完成配置。
- **预期**：行复选框是真正的切换（D441）：取消勾选只移除绑定、行保留在列表中，并在本次编辑会话内记住被移除的绑定，重新勾选即可恢复其参数——在每种发现模式下都成立，包括 fallback 列表与手动添加的模型：记住的绑定会合并回显示行；删除会丢弃这份记忆。右侧「移除」是真正的删除：它移除绑定并通过 `providers.create` / `providers.update` 把 id 持久化到提供商的 `hiddenModels` 集合、隐藏发现行，因此保存重开后模型不会重新出现。「显示」条目一次性恢复全部隐藏行，手输隐藏 id 也会解除隐藏；隐藏 id 永远不会隐藏仍持有绑定的行。
