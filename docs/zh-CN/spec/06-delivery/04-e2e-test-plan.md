...
- **步骤**：1) 每种状态各种植一行——选中、运行中、权限、ask、Plan/Goal 审批——
  ...
  2) 检查每行的前导状态圆点、它所在的胶囊，以及胶囊内的状态词。
...
- **预期**：运行中行显示橙色呼吸圆点（`--ds-warning`，
  ...
  在此之上，按 D445（落点由 D446 确定）：运行中行与三个待确认行把状态词放在
  圆点自己那枚淡彩胶囊里——胶囊同时装着圆点与文字，圆点在其中为静态定位，
  胶囊确有底色，且即使该行有两行高，这一组的中心仍与整行的垂直中心相差不超过
  2px，因此文字不可能漂到标题那一行上。文字与该圆点的可访问名称完全一致，且使用
  圆点自身的令牌（运行中为 `--ds-warning`，全部介入来源为 `--ds-purple`，
