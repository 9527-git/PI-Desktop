...
- **步骤**：1) 打开设置 → 模型配置并编辑该提供商。2) 确认左列表显示 fallback 提示，且列表标题旁渲染了全选复选框。3) 取消勾选全选框，确认没有任何模型被移除：每个已配置模型保持勾选（取消勾选是空操作）。4) 输入搜索过滤隐藏一个已配置模型，再勾选全选：只有可见行受影响，被隐藏的已配置模型保留绑定。5) 逐一点击左列表的每一行（id、行内空白、复选框），确认每个已配置模型保持勾选、没有任何模型被移除。6) 通过右侧面板的显式「移除」删除一个模型，保存并重开：其余绑定仍在。
- **预期**：头部全选在所有模式下都只加不删（D440）：它能添加可见行，但没有清空路径，因此 fallback 列表（恰好就是已配置的模型）绝不可能被引发回归的那个控件批量清空。删除只保留右侧面板的显式移除；如此删除的模型在 fallback 模式下可能不再出现在列表里，这正是任何位置都不提供批量破坏性控件的原因。
- **链接规格**：`04-ux/08-component-spec.md` §19.4、ADR 0192、D440
- **验收**：B（模型配置）
- **里程碑**：M2
