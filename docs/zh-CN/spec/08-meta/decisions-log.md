...
| D426 | 模型行复选框之外的点击永不删除模型 | **修订 ADR 0192 的选择器交互：在提供商模型选择器左侧列表中，复选框是唯一的切换控件。点击已配置行的 id、名称、行内空白或上限数值单元格会在右侧面板打开该模型的配置行、滚动到它并短暂标记该行；点击未配置的行等于勾选，因此行的手型光标永不删除模型。删除只保留复选框与右侧面板的移除控件。id 与名称仍是可选中文本，拖选仍是复制，键盘激活仍触发复选框的原生切换。仅渲染器变更；不改 i18n、IPC、协议、存储或 host-core。见 ADR 0192、E2E-005A 与 E2E-201。** | 行 label 在行内空白和上限数值上显示手型光标，但点击那里仍会激活复选框，导致想编辑模型的点击把它删除了。 |
| D427 | 跨会话渲染无需切换 | **三项相互关联的行为让属于其他会话的内容无需切换即可见。渲染器缓存为空的背景下会话改为从事件信封本身播种其实时尾部——投影对空的前序列表运行，因此中途打开该会话会显示流式行，而不只是持久化读取的内容；非转录事件不播种任何内容。子智能体停靠面板属于它的选中对象而不是可见会话：切换会话会保持它打开（其行在目标窗格的保留转录中重新定位），离开聊天页是唯一的关闭原因。宿主会话摘要新增最新的用户或助手文本——对 `messages` 表的关联子查询，扁平化、折叠空白、限制为 120 个码点加省略号，转录中没有可读文本时缺省——侧边栏每一行在标题下把它渲染为一行弱化且用 CSS 省略的文本。仅渲染器加一个增量宿主摘要字段；不改 schema 版本、协议版本、IPC 契约或权限。见 ADR 0256、E2E-CHAT-other-sessions-render-without-switching 与 US-UI-76。** | 侧边栏行无法说明会话的内容，中途打开的会话只显示持久化历史，而用户刚打开的停靠面板恰好在那个本应揭示其所属委派的切换上关闭了。 |
...
| D058 | 首页内容宽度+深色墨水标记 | *(宽度由 D429 修订：首页堆栈改用 868px 输入框长度标记)* **家庭双生长最大宽度使用`768px`（不是14px根下的`48rem`）；首页水平垫12px；英雄 title/night 控件使用主题令牌；夜间本垒板范围仅限于黑暗** | `48rem` 在 14px 根部收缩板 ~120px 与 Codex gold 相比；硬编码的光英雄墨水使夜间标题无法读取 |
...
| 身份证号 | 主题 | 决定 | 基本原理 |
|---|---|---|---|
| D101 | 受 WorkBuddy 启发的转录本密度 | **用户将渲染为紧凑的右对齐软板（`min(78%, 560px)`，微妙的边框+细线阴影）。助手将保持透明全角散文（最大 720 像素）。消息行垂直内边距收紧至 10 像素。 Hover/focus-within 在每圈下显示安静的复制芯片（用户右对齐，助理左对齐）。流媒体助理的答案使用左重音规则。还没有吉祥物、反应或成本芯片 UI。** *(由 D323 修订；720px 正文档上限由 D429 修订为 820px 的 `--chat-message-max-width` 标记)* | 与 WorkBuddy 的任务聊天相比，当前的右对齐用户气泡规格不足且视觉上稀疏；更密集的板提高了可扫描性，同时又不放弃 Codex/developer 约束 |
...
| D429 | 更宽的聊天带、全屏铺满与一条停靠接缝 | **居中聊天带加宽为 860px 内容 / 868px 输入框底板，助手消息列获得自己的 `--chat-message-max-width`（820px），因此 D101 的 720px 正文上限不再把整条带子钉在旧宽度上。侧边栏折叠时三个标记共用同一个更窄的上限：760px。`:root[data-fullscreen="true"]`（F11，由 `useAppShellRuntime` 发布）把三个标记都设为 `100%`，铺满整个主窗格；由于两条选择器特异性相同，全屏规则写在折叠规则之后由源码顺序取胜，样式测试断言了这一顺序。打开的工作面板重新获得且仅获得一条页内描边：聊天列右缘上 1px 的 `--ds-border-default` 细线，由主窗格（`.main-pane::after`，z-index 高于 `.conversation-topbar`）而不是停靠面板绘制，因为停靠面板自身的左侧盒边决定原生插件/浏览器视图预留，而这些原生视图合成在渲染器之上，会把画在面板内部的描边盖住；面板内部仍按 D297 保持无分隔线。仅渲染器 CSS：不改 IPC、宿主协议、存储、权限或依赖。见 `04-ux/07-ui-design-system.md` §6.4/§8.3、`04-ux/08-component-spec.md` §5.2 与 E2E-CHAT-fullscreen-widens-reading-band / E2E-WORKPANEL-open-dock-draws-chat-column-seam。** | 760/768px 的带宽源自一个 720px 正文上限，它已不再匹配宽屏上阅读转录的方式；而 D297 之后停靠面板的边界只剩色调差异。全屏时外壳占据整个显示器，阅读列理应不再为并不存在的界面预留空间。 |
...
- 在 D428 之前，重新发布未变化的 composer-dock-height 并不是免费的。
  该属性挂在 document.documentElement 上，因此每次 setProperty 都会使
  整个文档（包括转录区）的样式失效。D428 完全移除了该发布器；编辑器仍然
  只在计算值与上次应用值不同时写入高度和 overflowY。决定 D428 承接本地
  交付线的 D372 条目并移植到上游基线。
...
- 2026-09-16 由 D429 修订：恰好恢复一条页内描边——聊天列与打开的工作面板之间
  1px 的 `--ds-border-default` 细线；由于原生插件视图在该边缘合成在渲染器之上，
  它由主窗格绘制。此处余下的无边框规则（包括面板内部）全部继续有效。
...
## 2026-09-16 —— 模型行复选框之外的点击永不删除模型（D426）

- 选择器的实时列表整行显示手型光标，但只有复选框是切换控件。复选框之外的每次点击
  都是「打开」手势：点击已配置行的 id、名称、行内空白或上限数值会在右侧面板打开该
  模型的配置行、滚动到它并短暂标记；点击尚未配置的行等于勾选。删除只保留复选框与
  右侧面板的移除控件，因此为了编辑而做的点击永远不会删除模型。
- id 与名称仍是可选中文本：带选区的点击仍是复制，键盘产生的合成点击仍触发复选框的
  原生切换。定位标记是短暂的，减少动效时它是静态描边而不是动画。
- 仅渲染器变更：不改 i18n、IPC、协议、存储或 host-core。
- 决策 D426 在本地产线中承接 D397，并移植到上游基线上。见 ADR 0192、E2E-005A 与 E2E-201。

## 2026-09-16 —— 跨会话渲染无需切换（D427）

- 渲染器缓存为空的背景下会话此前会丢弃每个转录事件，因此中途打开它只能看到持久化读取返回的内容。现在缓存会从事件信封中自我播种：投影对空的前序列表运行，缓存像保留窗格一样累积流式尾部。非转录事件不播种任何内容，投影结果未变化时不写入。
- 子智能体停靠面板属于它的选中对象，而不是可见会话。切换会话会保持它打开——其行会在目标窗格的保留转录中重新定位——离开聊天页是唯一的关闭原因。面板中未使用的活动会话订阅和它的 sr-only 会话标题随旧门控一并移除。
- 宿主会话摘要新增最新的用户或助手文本：`SUMMARY_SELECT` 中对 `messages` 表的关联子查询，扁平化（换行转空格、折叠空白）、限制为 120 个码点加省略号，转录中没有可读文本时缺省。侧边栏每一行在标题下把它渲染为一行文本，弱化并在 `--text-2xs` 下用 CSS 省略。
- 仅新增宿主字段：不改 schema 版本、协议版本、IPC 契约或权限，渲染器仍是投影消费方。
- 决策 D427 在本地产线中承接 D326 / D328，并移植到上游基线上。见 `04-ux/07-ui-design-system.md` §16 与 E2E-CHAT-other-sessions-render-without-switching。

## 2026-09-16 —— 更宽的聊天带、全屏铺满与一条停靠接缝（D429）

- `.main-pane` 现在拥有三个宽度标记：`--chat-content-max-width` 860px、
  `--chat-composer-max-width` 868px，以及新增的 `--chat-message-max-width`
  820px。`messages.css` 中原本写死为字面量 `720px` 的每一处助手侧上限
  （13 处，含 `.review-change-card` 与 `.turn-summary-card`）都改为读取消息
  标记，因此正文列随带宽增长，而不是把带子钉住。用户气泡保持
  `min(max-content, 82%, 600px)` 形态。
- 侧边栏折叠时三个标记共用同一个更窄的上限：760px，取代此前 640px 的内容带，
  并保留更快的缓动。
- `:root[data-fullscreen="true"] .main-pane` 把三个标记都设为 `100%`，因此
  全屏外壳状态（F11，`documentElement.dataset.fullscreen`，由
  `useAppShellRuntime.tsx` 发布）让阅读列铺满窗格。它有意写在折叠覆盖规则之后：
  两者特异性相同，由源码顺序决定；`sidebar-collapse-animation.test.mjs` 同时
  断言取值与顺序。
- 停靠面板边界恢复一条细线。`.app-shell:has(> .work-panel)
  .main-pane::after` 在聊天列右缘渲染 1px 的 `--ds-border-default` 描边，
  `inset-block: 0`、`pointer-events: none`、`z-index: 11`，从而越过绝对定位的
  46px 会话顶栏（z-index 10）。它由主窗格而非停靠面板绘制：窗格右缘正是面板的
  左侧内容边，画在 `.work-panel` 内部的描边会被 `PluginViewTab`/浏览器宿主按
  该边缘定位并合成在渲染器之上的原生 `WebContentsView` 盖住。预览（最大化）
  模式卸载 `.main-pane`，因此那里不画接缝。面板内部仍按 D297 保持无分隔线；
  UI 设计系统 §6.4 把这条线记为唯一被重新引入的页内描边。
- 仅渲染器 CSS 加测试。不改 IPC、宿主协议、存储 schema、权限、依赖或任何持久化
  偏好；既有的面板宽度与缩放偏好不受影响。
- 决策 D429 修订 D058（首页底板宽度改为跟随 868px 输入框标记）、D101
  （助手正文档上限 720px → 820px）与 D297（一条停靠接缝）。见
  `04-ux/07-ui-design-system.md` §6.4 / §8.3、`04-ux/08-component-spec.md`
...
## 2026-09-18 —— 全选只加不删；接口地址与密钥快捷复制（D440）
...
> 同日由 D446 修订：文字保留，落点移进圆点自己的胶囊。当前契约以 D446 为准。
...
## 2026-09-18 —— 状态文字与圆点合进同一枚胶囊（D446）

> 同日由 D447 修订：点与词的配对保留不变，但胶囊退出标题所在的轨道。D449 随后又把它
> 带回标题所在的分组。当前契约请读 D449。

v0.14.34 构建之后用户反馈：D445 的文字「跑到会话标题最前面了」，读起来不像
状态灯的说明。原因是几何问题而非落点选错：圆点是绝对定位、垂直居中在整行上
（`sessions.css`），而自 D427 加上预览行后多数会话行有两行高。于是文字停在标题
那一行，灯浮在两行之间，两者不再像一个指示器。D445 的 CDP 夹具种的是单行行，
所以 16/16 全绿校验的是一个真实列表里根本不存在的布局。

- 两个需要关注的状态现在在行首渲染一枚淡彩状态胶囊：活的圆点与它的本地化状态词
  同处一个圆角分组（运行中 `--ds-warning` 14%、待确认 `--ds-purple` 14%，
  圆角用 `--radius-full`）。胶囊是行按钮的 flex 子元素，因此和原先的圆点一样
  垂直居中在整行上，任何行高下文字都无法与灯分离。
- 胶囊只在这两个状态上取代浮动的圆点。已选择、已完成、失败行仍是 D135 的留白区
  圆点，没有胶囊也没有文字；行首 20px 留白仅在存在胶囊时收紧为 6px
  （`.thread-item-main:has(.thread-item-status-chip)`），安静行完全不受影响。
- 胶囊内圆点放弃绝对定位与 3px 光环（强调交给淡底），但保留动画、`aria-label`
  与工具提示；文字仍标记 `aria-hidden`，读屏器每行仍只播报一个名称。未新增任何
  语言键——胶囊与 D445 一样复用 `nav.sessionRunning` 与 `nav.sessionNeedsInput`。
- 仅渲染器、文档与测试：不改 IPC 渠道、Host RPC、存储 schema、权限或持久化状态。
- 回归覆盖：capture rig 现在给每个种入的侧边栏行一条预览文字，于是
  `scripts/e2e-sidebar-status-dot.mjs` 量到的是真实的行高，并断言文字与圆点同处
  一枚胶囊、胶囊中心与行中心的偏差不超过 2px、胶囊确有淡底，且选中行的圆点仍是
  原本那个绝对定位的留白区圆点。`sidebar-session-status.test.mjs` 固定标记结构与
  令牌契约。
- 见 `04-ux/07-ui-design-system.md` §4.5、`04-ux/08-component-spec.md` §2 与 §3，
  以及 `06-delivery/04-e2e-test-plan.md` 的 E2E-SIDEBAR-status-dot-pending-union。

## 2026-09-18 —— 胶囊走在每行都保留的状态列里（D447）

> **取代 D446 的落点。** 点与词的配对、淡彩令牌、垂直居中、`aria-hidden` 孪生文字
> 全部沿用；改变的是胶囊坐在哪里。
>
> **次日由 D449 取代。** 这条列买到了标题对齐，代价是胶囊与自己的标题之间留出一条
> 死区，而且每一行的标题都要付 72px——无论它用不用得上标记。当前契约请读 D449。

v0.14.35 构建之后用户反馈：「状态胶囊不要挤标题啊，标题对齐，状态胶囊要在最前面
突出」。D446 把胶囊做成行按钮的 flex 子元素，于是关注行既释放了自己的 20px 留白
（`:has(.thread-item-status-chip) { padding-left: 6px }`），又把胶囊的宽度花在标题
前面：带胶囊的行标题画在 x≈85px，安静行的标题画在 x≈42px——两列参差不齐的标题，
而且胶囊吃掉了标题自身的宽度。

- 行现在通过 `.thread-item-main` 的 padding-left 预留一条前导**状态列**
  （`--ds-sidebar-status-column`，72px），无论该行有没有标记都预留同一条。因此
  运行中、待确认、已选择、已完成与安静行的会话标题共享同一个左边缘。
- 胶囊从内容流里移进这条列：它绝对定位在 `left: 4px`、垂直居中在整行上——和安静
  态圆点一直使用的锚点相同——所以胶囊领先于整行、仍是行上最醒目的元素，却永远
  进不了标题自己的轨道。它的宽度被限制在这条列内，因此过长的译文只会截断省略，
  推不动标题。胶囊与其文字为 `pointer-events: none`，只有圆点重新接回事件，整条列
  仍可点击以选中该行，圆点的工具提示也仍然可达。
- 胶囊必须把状态说进一个固定宽度里，所以状态词改成各语言的短形式
  （`nav.sessionRunningShort`、`nav.sessionNeedsInputShort`），已补进全部八个在
  用语言。完整短语（`nav.sessionRunning`、`nav.sessionNeedsInput`）留在圆点上作为
  工具提示与可访问名称，于是读屏器听到的内容没有缩水，眼睛看到的宽度刚好放得下。
- 安静状态保持不变：已选择、已完成、失败的行仍是绝对定位在该列左缘的 D135 圆点，
  没有胶囊也没有文字。
- 仅渲染器、语言包、文档与测试：不改 IPC 渠道、Host RPC、存储 schema、权限或持久
  化状态。
- 回归覆盖：`scripts/e2e-sidebar-status-dot.mjs` 现在逐行量出预留状态列的宽度与
  标题自身的左边缘，断言五颗种入的行共用同一个标题边缘，且每枚胶囊都从 `left: 4px`
  绝对定位、右边缘仍在标题左侧；「点词同组」断言额外要求胶囊文字不长于圆点的可访问
  名称、且没有被截断。`sidebar-session-status.test.mjs` 固定该列令牌、胶囊的绝对
...
## 2026-09-19 —— 状态标记领先所在行，顶在左边缘（D449）

> **取代 D447 的保留状态列。** D446 的点+词胶囊完整保留；变的是标记不再站在一条
> 自己的车道里，而是成为它自己标题分组的第一项。
>
> **同日勘误。** 标记改为领先「行自己的按钮」，而不是与按钮并排。已上线的几何请读
> D449 勘误。

v0.14.36 构建之后用户反馈：「状态要和标题在一起，中间不要有空档，可以和项目左侧
对齐，标题不能收缩」，随后「状态胶囊顶到左侧最边缘，最多不能超过一级项目边缘」。
在已上线的行上量下来：72px 的列把胶囊画在 x=4→61、标题画在 x=77——每个关注行都在
行内留出一条 11px 的死区，而 275px 的侧边栏里每一行标题（包括完全没有标记的行）
都要付 72px。这条列与上述约束互斥：任何固定车道要么留下一条死区（当标记比车道窄），
要么留下参差的边缘（当它不比车道窄）。用户选择了分组。

- 保留列消失。`--ds-sidebar-status-column` 变成 `--ds-sidebar-status-gap`（4px），
  `.thread-item-main` 回到普通的 `padding: 5px 6px`，`.thread-item` 用这条间隔令牌
  排布自己的子元素。
- 标记现在是行自身的第一个 flex 项。`.thread-item-status` 放弃
  `position: absolute; left: 4px; top: 50%; transform: translateY(-50%)` 改为随流
  排布，`.thread-item-status-chip` 用 `flex: 0 1 auto; min-width: 0` 换掉了绝对定位
  钉点与列宽上限。于是标记的左边缘就是行的左边缘——与一级项目标头起始的那条边同
  一条线——而过长的译文只在胶囊内部收缩省略，不会把标题推出行尾。
- D446 的配对不动：点与词仍在同一枚淡彩胶囊里，动画、`aria-label`、工具提示与完整
  短语仍归圆点所有，文字仍是 `aria-hidden`，安静状态仍是那枚没有胶囊的普通圆点。
- 已确认的代价：标题不再共享同一条左边缘。安静行的标题在 12px 圆点之后开始，关注行
  在自己的胶囊之后开始，而完全没有标记的行直接从左边缘起笔。这正是所要求的结果——
  状态与标题作为一个组移动，没有任何一行为一条永远用不到的车道付费。会话开始与停止
  运行时标题仍会移动；移动它的是胶囊。
- 仅渲染器、样式、文档与测试：不改 IPC 渠道、Host RPC、存储 schema、权限或持久化
  状态。未改任何语言键——D447 引入的短形式胶囊词保留，如今多担一份职责：让行首胶囊
  窄到能与自己的标题并肩。
- 回归覆盖：`sidebar-session-status.test.mjs` 固定间隔令牌、随流的点与胶囊、胶囊的
  收缩保护，以及被删掉的列确实已不存在；`scripts/e2e-sidebar-status-dot.mjs` 通过
  CDP 量出每枚标记的左边缘等于其行的左边缘、没有标记比一级项目标头更靠左，且每个
  标题都从自己标记右边缘起 6px 内开始。
- 见 `04-ux/07-ui-design-system.md` §4.5、`04-ux/08-component-spec.md` §2 与 §3，
  以及 `06-delivery/04-e2e-test-plan.md` 的 E2E-SIDEBAR-status-dot-pending-union。

## 2026-09-19 —— 标记领先行自己的按钮（D449 同日勘误）

D449 的第一次落地把标记移出行按钮，却把该按钮自己的 `padding-left: 6px` 留在原地，
于是刚刚在对比页上被批准的几何被两处破坏：

- 胶囊→标题的空档变成 10px（4px 行间隔再加上按钮的 6px 内边距），而 N1 面板量出来
  是 4px——仍然是一条死区，而这正是对 D447 的抱怨本身。
- 标记不再属于按钮，于是点胶囊或状态词什么都选不中。D447 之所以没有这个问题，是因为
  它给按钮 72px 的左内边距、把整条列盖在按钮里；这次行的点击目标缩了约 60px。

标记现在改为行按钮自己的第一个 flex 项，并且按钮的内容盒从行的左边缘起笔
（`padding: 5px 6px 5px 0`、`gap: var(--ds-sidebar-status-gap)`）。盒模型算出来就是
被批准的那版——标记顶在 x=0、标题在其后 4px——而整行（含状态）仍是选中会话的控件。

- 仅渲染器、样式、文档与测试：不改 IPC 渠道、Host RPC、存储 schema、权限或持久化
  状态。
- 回归覆盖：`sidebar-session-status.test.mjs` 现在钉住标记渲染在 `thread-item-main`
  之内，且该按钮的左内边距为 0。
- 见 `04-ux/07-ui-design-system.md` §4.5。
