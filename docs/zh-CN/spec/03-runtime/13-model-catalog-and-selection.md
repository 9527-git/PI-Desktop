...
- 激活单个发现的模型是加性操作，不是移除开关。它的文本、行内边距、限制与复选框都在模型缺失时添加，或打开其既有配置。重复、双击、键盘与合成点击都保留每个既有绑定，包括其位置、别名与覆盖；大小写变体不会产生重复。若右侧已选面板的过滤会隐藏被激活的模型，先清除该过滤再展示其设置。移除单个模型必须使用已选面板中它的显式「移除」操作。单独标注的批量选择/清空控件保持其既有作用范围。
