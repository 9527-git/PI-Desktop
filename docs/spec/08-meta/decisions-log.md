...
| D424 | Open and reveal local paths outside the workspace | **Amend D320 / D322 / ADR 0109 / ADR 0111: chat file references recognize Windows drive paths, backslash separators, and MSYS mounts, and an absolute path outside the workspace resolves to an external target that opens through the OS. `fs/open` and `fs/reveal` accept an existing local absolute path behind a realpath plus protected-roots (app data directory) guard, with workspace containment keeping priority; `fs/reveal` selects a file in its folder, opens a directory, and falls back to the nearest existing ancestor so a vanished entry is never a silent no-op. A new additive channel `pi-desktop/fs/stat` reports the verdict and entry kind only. `fs/read`, `fs/list`, `fs/index`, and every agent-tool path gate are unchanged. See ADR 0256, ADR 0257, and E2E-CHAT-external-path-open-reveal.** | Artifact paths in assistant answers routinely live outside the workspace, so workspace-only containment made the transcript's path references dead ends (D320 / D322). |
| D425 | Per-turn summary card with changes and outputs | **Add a renderer-only summary card under every completed (non-active) turn: the step count and the workspace files the turn changed, grouped by added/modified/deleted with per-file diffs, plus the output paths its final answer reported — which covers artifacts no Write/Edit record can see. Long lists render as two tables (changes, outputs) with columns for what changed, the path, and the row actions (open in the work panel, show in the file manager). `fs/stat` (D424 / ADR 0257) marks a row whose path no longer resolves and disables its actions, so a stale reference reads as stale instead of as a dead button. The card reads the turn's own tool results and answer; it does not call git, and it adds no permission, IPC, host-core, storage, or schema change beyond the `fs/stat` probe. See ADR 0257 and E2E-CHAT-turn-summary-card.** | A finished turn scattered its outcome across the activity rows, so the reader had to reconstruct what changed and what was produced; the turn boundary already owns that information. |
...
| D426 | Model row clicks outside the checkbox never drop the model | **Amend ADR 0192's picker interaction: in the provider model picker's left list, the checkbox is the only toggle. A click on a configured row's id, name, padding, or limits cell opens that model's configuration row in the right pane, scrolls it into view, and marks it briefly; a click on an unconfigured row picks it, so the row's pointer cursor never removes a model. Removal stays with the checkbox and the right pane's remove control. The id and name remain selectable text and a drag-selection stays a copy, while keyboard activation keeps the checkbox's native toggle. Renderer-only; no i18n, IPC, protocol, storage, or host-core change. See ADR 0192, E2E-005A, and E2E-201.** | The row label showed a hand cursor over its padding and limits cell while a click there still activated the checkbox, so a click made to edit a model removed it. |
| D427 | Cross-session rendering without switching | **Three connected behaviors make content that belongs to another session visible without switching to it. A background session whose renderer cache was empty seeds its live tail from the event envelope itself — the projection runs against an empty previous list, so opening a session mid-run shows the streaming rows instead of only the durable read; non-transcript events seed nothing. The subagent dock belongs to its selection rather than to the visible session: a session switch keeps it open (its rows re-find in the destination pane's retained transcript), and leaving the chat page is the only close reason. The host session summary carries the newest user or assistant text — a correlated subquery over the messages table, flattened, whitespace-collapsed, bounded to 120 code points plus an ellipsis, and absent when the transcript has no readable text — and each sidebar row renders it as one muted, CSS-ellipsized line under the title. Renderer plus one additive host summary field; no schema version, protocol version, IPC contract, or permission change. See ADR 0256, E2E-CHAT-other-sessions-render-without-switching, and US-UI-76.** | A sidebar row could not say what a session was about, an opened mid-run session showed only its durable history, and the dock the user had just opened closed on exactly the switch that was supposed to reveal the delegate it belonged to. |
...
- Before D428, republishing an unchanged `--composer-dock-height` was not free.
  The property lived on `document.documentElement`, so every `setProperty`
  invalidated style for the whole document, including the transcript. D428 removes
  that publisher entirely; the editor still writes height and `overflowY` only
  when the computed value differs from what was last applied. Decision D428
  follows the local line's D372 entry and is ported onto the upstream base.
...
## 2026-09-15 — Chat paths outside the workspace open, reveal, and report staleness (D424)

- The transcript recognizes Windows drive paths, backslash separators, and
  MSYS mounts in chat text. An absolute path outside the workspace resolves to
  an external target and opens through the OS instead of the work-panel
  viewer, so an artifact path in an assistant answer is no longer a dead end.
- `fs/open` and `fs/reveal` accept an existing local absolute path behind a
  realpath plus protected-roots guard (the app data directory is refused).
  Workspace and session-root containment keeps priority and its behavior is
  unchanged. `fs/read`, `fs/list`, `fs/index`, and every agent-tool gate stay
  workspace-scoped: the relaxation covers only the two user-initiated OS
  handoff actions.
- `fs/reveal` selects a file in its folder, opens a directory, and falls back
  to the nearest existing ancestor when the entry disappeared between the
  containment check and the handoff, so the action is never a silent no-op on
  Windows.
- The additive channel `pi-desktop/fs/stat` reports `{ exists, kind }` only —
  no content, size, or listing — through the same gates, so a row whose path no
  longer resolves reads as stale instead of as a dead button.
- Decision D424 amends D320 / D322 and ADR 0109 / 0111. See ADR 0256,
  ADR 0257, and E2E-CHAT-external-path-open-reveal.

## 2026-09-15 — Per-turn summary card (D425)

- A completed turn ends with one compact card in the transcript instead of
  leaving the reader to reconstruct the outcome from the activity rows. It is
  rendered only for a turn that is no longer active, so live output is never
  covered by a summary of itself.
- **Changes** lists one row per workspace file the turn durably changed:
  added / modified / deleted, the operation and its +/− counts, the path, and
  per-file diff expansion. **Outputs** lists the paths the turn's final answer
  reported as produced, with the reported line, which covers artifacts no
  Write/Edit record can see.
- Each row can open the path in the work panel or show it in the file manager.
  Both actions reuse the existing chat-path handoff (D424 / ADR 0256), so a
  path outside the workspace opens through the OS.
- A row whose path no longer resolves reports the not-found hint beside the
  path and disables its actions. The verdict comes from the read-only
  `pi-desktop/fs/stat` probe (ADR 0257), and an unreachable probe counts as
  not-found, because the renderer has no other evidence.
- The card reads the turn's own tool results and final answer. It does not call
  git, so a message's evidence stays the only source of a change claim.
- Renderer-only apart from the existing probe: no new permission, no
  host-core, storage, or schema change.
- Decision D425 follows D408 / D409 on the local line and is ported onto the
  upstream base. See ADR 0256, ADR 0257, and E2E-CHAT-turn-summary-card.

## 2026-09-16 — Model row clicks outside the checkbox never drop the model (D426)

- The picker's live list shows a pointer cursor over the whole row, but only the
  checkbox is a toggle. Every click outside it is an open gesture: a configured
  row's id, name, padding, and limits cell open that model's configuration row
  in the right pane, scroll it into view, and mark it briefly; a click on a row
  that is not configured yet picks it. Removal stays with the checkbox and the
  right pane's remove control, so a click made to edit a model can never remove
  it.
- The id and name stay selectable text: a click that carries a selection remains
  a copy, and the keyboard's synthetic click still runs the checkbox's native
  toggle. The reveal mark is transient, and under reduced motion it is a static
  outline instead of an animation.
- Renderer-only: no i18n, IPC, protocol, storage, or host-core change.
- Decision D426 follows D397 on the local line and is ported onto the upstream
  base. See ADR 0192, E2E-005A, and E2E-201.

## 2026-09-16 — Cross-session rendering without switching (D427)

- A background session whose renderer cache was empty used to drop every
  transcript event, so opening it mid-run showed only what the durable read
  returned. The cache now seeds itself from the event envelope: the projection
  runs against an empty previous list and the cache accumulates the streaming
  tail exactly as a retained pane does. Non-transcript events seed nothing, and
  an unchanged projection writes nothing.
- The subagent dock belongs to its selection, not to the visible session.
  Switching sessions keeps it open — its rows re-find in the destination pane's
  retained transcript — and leaving the chat page is the only close reason. The
  panel's unused active session subscription and its sr-only session title go
  with the old gate.
- The host session summary gains the newest user or assistant text: a
  correlated `messages` subquery in `SUMMARY_SELECT`, flattened (newlines to
  spaces, whitespace collapsed), bounded to 120 code points plus an ellipsis,
  and absent when the transcript has no readable text. Each sidebar row renders
  it as one line under the title, muted and CSS-ellipsized at
  `--text-2xs`.
- Additive host field only: no schema version, protocol version, IPC contract,
  or permission change, and the renderer stays a projection consumer.
