...
| D397 | Model row clicks outside the checkbox never drop the model | **Amend ADR 0192's picker interaction: in the provider model picker's left list, the checkbox is the only toggle. A click on a configured row's id, name, padding, or limits cell opens that model's configuration row in the right pane, and a click on an unconfigured row picks it; the row's pointer cursor never removes a model. Removal stays with the checkbox and the right pane's remove control. The id and name remain selectable text and a drag-selection stays a copy. Renderer-only; no i18n, IPC, protocol, or storage change. See E2E-005A.** | The row label showed a hand cursor over its padding and limits cell while a click there still activated the checkbox, so a click made to edit a model removed it. |
...
- Before D372, republishing an unchanged `--composer-dock-height` was not free.
  The property lived on `document.documentElement`, so every `setProperty`
  invalidated style for the whole document, including the transcript. D372 removes
  that publisher entirely; the editor still writes height and `overflowY` only
  when the computed value differs from what was last applied.
