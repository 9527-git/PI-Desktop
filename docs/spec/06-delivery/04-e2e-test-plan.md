...
- **Expected**: The transcript immediately shows a compact localized `Working…`
  ...
  when the turn ends.
  While that turn runs, the session's sidebar row shows its orange breathing
  dot, and clears it when the turn ends (E2E-SIDEBAR-status-dot-pending-union).
...
- **Expected**: Every route-owned top region uses the same `--ds-toolbar-height`
  ...
  conversation top bar renders with its title cluster — the task title and no
  per-session status indicator (D444 moved run/needs-input state to the sidebar
  row dot) — and actions only; it has no model
...
#### E2E-SIDEBAR-status-dot-pending-union: The sidebar row dot tracks run state and the human-intervention union

- **Preconditions**: PI-Desktop is open with at least five sessions, and a built
...
- **Steps**: 1) Seed one row per state — selected, running, permission, ask,
  Plan/Goal approval. 2) Inspect each row's leading status dot. 3) Repeat the
  running and needs-input rows in light and dark themes. 4) Re-inspect under
...
- **Expected**: The running row shows an orange breathing dot (`--ds-warning`,
  `sidebar-status-breathe`). The permission, ask, and Plan/Goal rows all render
  the SAME needs-input dot — class `permission`, purple (`--ds-purple`,
  `sidebar-status-pulse-purple`) — proving the D444 union, and share one
  accessible label (`nav.sessionNeedsInput`) distinct from the running label.
  ...
  preference restores them. The conversation topbar has no `.ct-status-slot` or
...
- **Preconditions**: PI-Desktop is open on a chat session where at least one
  full turn (user prompt, assistant reply with model usage) is recorded.
- **Steps**: 1) With the session idle, inspect the meta row under the
  assistant reply. 2) Send another prompt and inspect the meta row while the
  turn is streaming, then after it settles. 3) Inspect the row under the user
  bubble. 4) Hover each chip.
- **Expected**: The settled assistant meta row shows the model chip plus
  input, output, and (when the provider reports them) cache-read and
  cache-write chips with compact token counts (`12.3k`), summed across the
  turn's messages. The meta row also ends with the reply time (clock time
  today, a short date plus time otherwise). User messages show a time chip
  under the bubble; while a turn streams no usage chips appear, and a turn
  without usage data still shows its model and time. Chips are non-
  interactive text with full-precision values in the tooltip. No IPC, host,
  or persisted state is touched.
...
- **Steps**: 1) Open Settings → Model configuration and reopen the provider.
  ...
  and confirm it fails with `MODEL_ALIAS_TOO_LONG`. 10) Click that chosen row's
  id in the live list, then its row padding and its limits cell; confirm each
  click keeps the model configured, opens its Advanced body, and scrolls the
  right pane to that row. 11) Click a row whose model is not configured;
  confirm that row's checkbox becomes checked and its configuration row
  appears.
...
- **Preconditions**: A named AI service saved with an API key (and, for the
  negative case, a provider edited down to no stored key).
- **Steps**: 1) Edit the provider and confirm the endpoint line carries a
  copy button. 2) Press it and paste elsewhere: the full base URL is on the
  clipboard. 3) Confirm the API-key field carries a copy button in edit mode.
  4) Press it and paste: the stored key is on the clipboard, and a success
  toast appeared. 5) For a provider without a stored key, press the key copy
  button: a miss toast appears and the clipboard keeps its previous content.
- **Expected**: The endpoint copy writes the full base URL through the
  renderer clipboard. The key copy never sends the secret to the renderer:
