...
lives on the sidebar session row status indicator — leading dot plus inline
status word (D444, D445, §3) — so the title cluster stays a single, calm lane.
...
- The top bar exposes no status live region; per-session run and needs-input
  ...
  (D444, §3). The inline status word on that row is the dot's visual twin and is
  `aria-hidden`, so a screen reader still hears one label per row (D445). The
...
| Inline session status word | `--text-2xs` (11px) | Secondary annotation before the title, same tier as the row preview; only the in-progress and needs-input rows carry it (D445) |
...
| Session needs input | Purple pulsing dot **plus** the purple status word (`nav.sessionNeedsInput`) inline before the title, when a permission request, asktool question, or Plan/Goal approval awaits the user; outranks in-progress; dot static under reduced motion (D444, D445) |
| Session in progress | Orange breathing dot **plus** the orange status word (`nav.sessionRunning`) inline before the title; dot static under reduced motion (D445) |
| Session completed | Green check mark from the latest unread task notification when the row is not selected; dot only, no inline word (D445) |
| Session failed | Red circled alert mark from the latest unread task notification when the row is not selected; dot only, no inline word (D445) |
...
- Every visible session indicator has a localized accessible name and tooltip;
  ...
  attention states add a localized status word inline before the title so their
  meaning never rests on hue alone (D445)
