...
test("every shipped locale carries the summary labels", async () => {
  ...
  for (const locale of LOCALES) {
    ...
    // de / es / fr quote every key in their catalog; the other locales use
    // bare identifiers. The contract is that the labels are present, not how
    // each catalog spells its keys.
    assert.match(text, /"?summaryOp"?: {/, `${locale} is missing summaryOp`);
    for (const op of ["added", "modified", "deleted"]) {
      assert.ok(
        text.includes(`${op}:`) || text.includes(`"${op}":`),
        `${locale} is missing summaryOp.${op}`,
      );
    }
  }
});
