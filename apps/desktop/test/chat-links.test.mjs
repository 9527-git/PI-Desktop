...
// The public entry point is a facade over `chat-links/`, so the hook supplies
// the `.ts` extension its extensionless relative imports omit.
...
const {
  ...
  splitChatText,
  toWorkspaceRel,
} = await import("../src/lib/chat-links.ts");
...
test("resolvePreviewTarget marks external drive paths and keeps posix strictness", () => {
  ...
  // POSIX absolute paths outside the root stay unresolvable: a `/usr/...`
  // token in prose is far more likely to be an example than a file.
...
test("the scanner stays linear on a hostile path-like string", () => {
  // ReDoS guard: the segment class excludes separators, so this cannot
  // backtrack exponentially.
  const hostile = "a/".repeat(4000) + "\\".repeat(4000);
  const started = performance.now();
  splitChatText(hostile, ROOT);
  parseFileRef(hostile);
  assert.ok(performance.now() - started < 1000, "scan must not backtrack");
});
